...
type NotificationItem = {
  ...
  message: string;
  timeLabel: string;
  statusColor: "bg-success-500" | "bg-warning-500" | "bg-error-500";
  imageSrc: string;
};
...
const getDisplayName = (row: ProfileRow) => {
  ...
  const joinedName = `${row.first_name ?? ""} ${row.last_name ?? ""}`.trim();
  if (joinedName) return joinedName;

  return row.email?.split("@")[0] || "User";
};

const getTimeAgo = (dateText: string | null) => {
  if (!dateText) return "unknown time";

  const created = new Date(dateText).getTime();
  if (Number.isNaN(created)) return "unknown time";
...
const getAvatarByRole = (role: AppRole) => {
  if (role === "guru") return "/images/user/user-03.jpg";
  if (role === "admin") return "/images/user/user-05.jpg";
  return "/images/user/user-02.jpg";
};

const getStatusByRole = (role: AppRole): NotificationItem["statusColor"] => {
  if (role === "guru") return "bg-success-500";
  if (role === "admin") return "bg-error-500";
...
export default function NotificationDropdown() {
  ...
  useEffect(() => {
    const loadNotifications = async () => {
        ...
        setNotifications([
          {
            id: "welcome",
            title: "Signed In",
            message: `You are logged in as ${user.email ?? "user"}`,
            timeLabel: "just now",
            statusColor: "bg-success-500",
            imageSrc: "/images/user/owner.jpg",
          },
        ]);
        setNotifying(true);
        return;
      ...
      const rows = (data as ProfileRow[]).filter((row) => row.id !== user.id);
      const mapped: NotificationItem[] = rows.map((row) => {
        const humanRole = roleLabel(row.role);

        return {
          id: row.id,
          title: getDisplayName(row),
          message: `New ${humanRole} account was created`,
          timeLabel: getTimeAgo(row.created_at),
          statusColor: getStatusByRole(row.role),
          imageSrc: getAvatarByRole(row.role),
        };
      });
        ...
        setNotifications([
          {
            ...
            message: "There are no account updates right now",
            timeLabel: "just now",
            statusColor: "bg-success-500",
            imageSrc: "/images/user/owner.jpg",
          },
            ...
            <li key={item.id}>
              ...
              >
                <span className="relative z-1 block h-10 w-full max-w-10 rounded-full">
                  ...
                  />
                  <span
                    className={`absolute bottom-0 right-0 z-10 h-2.5 w-full max-w-2.5 rounded-full border-[1.5px] border-white ${item.statusColor} dark:border-gray-900`}
                  ></span>
                </span>
